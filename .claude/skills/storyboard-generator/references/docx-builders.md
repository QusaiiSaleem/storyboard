...
Complete API documentation for all 8 DOCX builders in `scripts/docx_engine.py`.
...
| `set_image(image_path, image_prompt)` | `image_path: str`, `image_prompt: str` | Set hero image. Priority: image_path > image_prompt |

### Builders in This Group

- **ObjectivesBuilder** — Learning Objectives (الأهداف التعليمية)
- **SummaryBuilder** — Unit Summary (الملخص)
- **InfographicBuilder** — Learning Map / Infographic (خارطة التعلم)

### Example (ObjectivesBuilder — same pattern for all three)

```python
from docx_engine import ObjectivesBuilder

builder = ObjectivesBuilder(project_code="DSAI", unit_number=1, unit_name="المهارات الرقمية",
    project_name="تطوير 15 مقرر إلكتروني - جامعة نجران", institution="جامعة نجران", designer="أحمد")
builder.set_element_name("الأهداف التعليمية")
builder.set_screen_description("وصف شاشة الأهداف التعليمية")
builder.set_content_text("1. أن يُعرّف المتعلم مفهوم التقنية الناشئة\n2. أن يُحدد المتعلم فوائد التقنية الرقمية")
...
- **DiscussionBuilder** — Discussion (النقاش)
- **AssignmentBuilder** — Assignment (الواجب)

### Example (DiscussionBuilder — same pattern for AssignmentBuilder)

```python
from docx_engine import DiscussionBuilder

builder = DiscussionBuilder(project_code="DSAI", unit_number=1, unit_name="المهارات الرقمية",
    project_name="تطوير 15 مقرر إلكتروني - جامعة نجران", institution="جامعة نجران", designer="أحمد")
...
builder = TestBuilder(project_code="DSAI", unit_number=1, unit_name="المهارات الرقمية",
    project_name="تطوير 15 مقرر إلكتروني - جامعة نجران", institution="جامعة نجران", designer="أحمد")
builder.set_element_name("الاختبار القبلي")
builder.set_test_info(description="الاختبار القبلي للوحدة الأولى", instructions="المحاولات المتاحة: محاولة واحدة")
builder.add_question(
    question_text="ما هو الذكاء الاصطناعي؟",
    choices="أ) برنامج حاسوبي\nب) فرع من علوم الحاسب\nج) لغة برمجة\nد) نظام تشغيل",
    correct_answer="ب", image_description="")
builder.build()
...
- `image_prompt: str` — AI image generation prompt (English, optional)
...
builder = ActivityBuilder(project_code="DSAI", unit_number=1, unit_name="المهارات الرقمية",
    project_name="تطوير 15 مقرر إلكتروني - جامعة نجران", institution="جامعة نجران", designer="أحمد")
builder.set_element_name("النشاط التفاعلي 1.1")
builder.add_scene(title="المشهد الأول", description="في هذا المشهد يظهر للطالب سؤال...",
    elements="يظهر السؤال مع أربعة خيارات...", image_desc="صورة توضيحية للمفهوم",
...
builder = VideoBuilder(project_code="DSAI", unit_number=1, unit_name="المهارات الرقمية",
    project_name="تطوير 15 مقرر إلكتروني - جامعة نجران", institution="جامعة نجران", designer="أحمد")
builder.set_element_name("فيديو موشن الوحدة 1")
builder.add_scene(title="مشهد العنوان", screen_description="يظهر العنوان الرئيسي مع شعار الجامعة",
    sound_effects="موسيقى هادئة", narration_segments=[
        {"narration": "مرحبا بكم في الوحدة الأولى...", "on_screen_text": "المهارات الرقمية",
         "scene_description": "بالتزامن مع النص يظهر العنوان...", "image_links": "logo.png"},
    ])
builder.build()
